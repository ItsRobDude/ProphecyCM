...
def test_process_turn_retains_original_ally_ids_when_hydrated_from_meta():
    rng = random.Random(9)
    pc = build_pc()
    companion = build_companion("ally-4", dex=14)
    enemy = build_creature("bandit-2", dex=10)

    encounter = start_encounter("enc-ally-id", pc, [enemy], rng, allies=[companion])
    encounter_loaded = EncounterState.from_dict(encounter.to_dict())

    ally_ref = CombatantRef("npc", companion.id)
    enemy_ref = CombatantRef("creature", enemy.id)
    encounter_loaded.active_index = next(i for i, entry in enumerate(encounter_loaded.turn_order) if entry.ref == ally_ref)

    attack_action = CreatureAction(name="Slash", attack_ability="strength", damage_dice="1d8", to_hit_bonus=12)
    command = {"type": "attack", "target": enemy_ref, "action": attack_action}

    result = process_turn_commands(encounter_loaded, pc, [enemy], [command], rng)

    assert result.status in {"ongoing", "victory"}
    assert enemy.current_hit_points < enemy.hit_points
    assert result.log and result.log[0].actor == ally_ref
