...
- `src/prophecycm/combat/`: status-effect rules (stacking, durations, dispels) plus the lightweight combat engine (initiative, attack resolution, consumable usage, encounter state).
- `src/prophecycm/quests/`: quest graph primitives (conditions, effects, steps) with lightweight evaluation helpers.
- `src/prophecycm/dialogue/`: dialogue node/choice contracts and a runner for gating + effects.
- `src/prophecycm/world/`: location graph, encounter tables, and travel constraints.
- `src/prophecycm/state/`: authoritative `GameState` + `SaveFile` containers and helper methods for time/flags/travel/encounters, including faction/relationship reputation and active encounter tracking.
...
- Extend combat to include richer actions/AP, positional rules, and player command handling.
- Expand the dialogue/choice runner to integrate quest effects, relationship/faction shifts, and UI transcripts.
